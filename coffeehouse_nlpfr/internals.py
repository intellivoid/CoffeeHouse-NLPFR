...
from coffeehouse_nlpfr import compat
...
def config_java(bin=None, options=None, verbose=False):
    """
    Configure coffeehouse_nlpfr's java interface, by letting coffeehouse_nlpfr know where it can
    ...
    :param bin: The full path to the Java binary.  If not specified,
        then coffeehouse_nlpfr will search the system for a Java binary; and if
...
def read_str(s, start_position):
    ...
    :Example:
    >>> from coffeehouse_nlpfr.internals import read_str
...
def read_int(s, start_position):
    ...
    :Example:
    >>> from coffeehouse_nlpfr.internals import read_int
...
def read_number(s, start_position):
    ...
    :Example:
    >>> from coffeehouse_nlpfr.internals import read_number
...
def deprecated(message):
    ...
    a warning to be printed the when the function is used.  Usage:
        ...
        >>> from coffeehouse_nlpfr.internals import deprecated
...
def find_file_iter(
    ...
    """
    Search for a file to be used by coffeehouse_nlpfr.
...
def find_jar_iter(
    ...
    """
    Search for a jar that is used by coffeehouse_nlpfr.
...
def import_from_stdlib(module):
    """
    When python is run from within the coffeehouse_nlpfr/ directory tree, the
    current directory is included at the beginning of the search path.
    Unfortunately, that means that modules within coffeehouse_nlpfr can sometimes
