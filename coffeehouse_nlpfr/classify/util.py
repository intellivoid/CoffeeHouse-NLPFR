...
# from coffeehouse_nlpfr.util import Deprecated
import coffeehouse_nlpfr.classify.util  # for accuracy & log_likelihood
...
class CutoffChecker(object):
    ...
    def check(self, classifier, train_toks):
        ...
        new_ll = coffeehouse_nlpfr.classify.util.log_likelihood(classifier, train_toks)
        ...
        if "max_acc" in cutoffs or "min_accdelta" in cutoffs:
            new_acc = coffeehouse_nlpfr.classify.util.log_likelihood(classifier, train_toks)
...
def names_demo(trainer, features=names_demo_features):
    from coffeehouse_nlpfr.corpus import names
    import random
...
def wsd_demo(trainer, word, features, n=1000):
    from coffeehouse_nlpfr.corpus import senseval
    import random
...
def check_megam_config():
    ...
    try:
        ...
        err_msg = str(
            ...
            ">>> coffeehouse_nlpfr.config_megam('/usr/bin/local/megam')"
        )
