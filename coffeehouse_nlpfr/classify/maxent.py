...
In the rest of the ``coffeehouse_nlpfr.classify`` module, the term "features" is
...
from coffeehouse_nlpfr.data import gzip_open_unicode
from coffeehouse_nlpfr.util import OrderedDict
from coffeehouse_nlpfr.probability import DictionaryProbDist

from coffeehouse_nlpfr.classify.api import ClassifierI
from coffeehouse_nlpfr.classify.util import CutoffChecker, accuracy, log_likelihood
from coffeehouse_nlpfr.classify.megam import call_megam, write_megam_file, parse_megam_weights
from coffeehouse_nlpfr.classify.tadm import call_tadm, write_tadm_file, parse_tadm_weights
...
def train_maxent_classifier_with_megam(
    ...
    :see: ``train_maxent_classifier()`` for parameter descriptions.
    :see: ``coffeehouse_nlpfr.classify.megam``
    """
    ...
    try:
        fd, trainfile_name = tempfile.mkstemp(prefix="coffeehouse_nlpfr-")
...
class TadmMaxentClassifier(MaxentClassifier):
    ...
    def train(cls, train_toks, **kwargs):
        ...
        trainfile_fd, trainfile_name = tempfile.mkstemp(
            prefix="coffeehouse_nlpfr-tadm-events-", suffix=".gz"
        )
        weightfile_fd, weightfile_name = tempfile.mkstemp(prefix="coffeehouse_nlpfr-tadm-weights-")
...
######################################################################
def demo():
    from coffeehouse_nlpfr.classify.util import names_demo
