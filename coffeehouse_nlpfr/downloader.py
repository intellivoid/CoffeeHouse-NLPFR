...
Usage::
    ...
    python coffeehouse_nlpfr/downloader.py [-d DATADIR] [-q] [-f] [-k] PACKAGE_IDS

or::

    python -m coffeehouse_nlpfr.downloader [-d DATADIR] [-q] [-f] [-k] PACKAGE_IDS
"""
# ----------------------------------------------------------------------
...
* Should the data files be kept in svn at all?  Advantages: history;
  ...
  people to have to download 400MB corpora just to use coffeehouse_nlpfr from svn.

* Compromise: keep the data files in trunk/data rather than in
  trunk/coffeehouse_nlpfr.  That way you can check them out in svn if you want
...
try:
    ...
    from coffeehouse_nlpfr.draw.util import ShowText
except ImportError:
    TKINTER = False
    TclError = ValueError
...
# urllib2 = coffeehouse_nlpfr.internals.import_from_stdlib('urllib2')
...
class Downloader(object):
    ...
    def _update_index(self, url=None):
        ...
        self._index = coffeehouse_nlpfr.internals.ElementWrapper(
    ...
    def default_download_dir(self):
        ...
        ``PYTHONHOME/lib/coffeehouse_nlpfr``, where *PYTHONHOME* is the
        ...
        for nltkdir in coffeehouse_nlpfr.data.path:
            if os.path.exists(nltkdir) and coffeehouse_nlpfr.internals.is_writable(nltkdir):
                return nltkdir
...
# Helper Functions
######################################################################
# [xx] It may make sense to move these to coffeehouse_nlpfr.internals.
...
def _find_packages(root):
    ...
    """
    from coffeehouse_nlpfr.corpus.reader.util import _path_from
