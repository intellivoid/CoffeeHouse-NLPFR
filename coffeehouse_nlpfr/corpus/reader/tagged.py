...
from coffeehouse_nlpfr.tag import str2tuple, map_tag
...
from coffeehouse_nlpfr.corpus.reader.timit import read_timit_block
...
class TaggedCorpusReader(CorpusReader):
    ...
    using ``coffeehouse_nlpfr.tag.str2tuple``.  By default, ``'/'`` is used as the
...
class TaggedCorpusView(StreamBackedCorpusView):
    ...
    ``TaggedCorpusReader`` (not directly by coffeehouse_nlpfr users).
    """
