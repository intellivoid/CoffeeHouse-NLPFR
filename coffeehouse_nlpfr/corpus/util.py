...
class LazyCorpusLoader(object):
    ...
    package (or modified ``coffeehouse_nlpfr.data.path`` to point to its location),
    ...
    def __load(self):
        ...
        if TRY_ZIPFILE_FIRST:
            try:
                root = coffeehouse_nlpfr.data.find("{}/{}".format(self.subdir, zip_name))
            except LookupError as e:
                try:
                    root = coffeehouse_nlpfr.data.find("{}/{}".format(self.subdir, self.__name))
                except LookupError:
                    raise e
        else:
            try:
