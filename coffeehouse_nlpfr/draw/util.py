...
The ``coffeehouse_nlpfr.draw`` module defines the abstract ``CanvasWidget`` base
class, and a number of simple canvas widgets.  The remaining canvas
widgets are defined by submodules, such as ``coffeehouse_nlpfr.draw.tree``.

The ``coffeehouse_nlpfr.draw`` module also defines ``CanvasFrame``, which
...
class CanvasWidget(object):
    ...
    arguments of the form ``attribute=value``:
        ...
        >>> from coffeehouse_nlpfr.draw.util import TextWidget
