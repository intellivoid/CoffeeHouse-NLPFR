...
using URLs, such as ``coffeehouse_nlpfr:corpora/abc/rural.txt`` or
...
supported:
  ...
  - ``coffeehouse_nlpfr:path``: Specifies the file stored in the NLTK data
    package at *path*.  NLTK will search for these files in the
    directories specified by ``coffeehouse_nlpfr.data.path``.

If no protocol is specified, then the default protocol ``coffeehouse_nlpfr:`` will
be used.
...
from coffeehouse_nlpfr.compat import py3_data, add_py3_data, BytesIO
...
def split_resource_url(resource_url):
    ...
    >>> split_resource_url('coffeehouse_nlpfr:home/coffeehouse_nlpfr')
    ('coffeehouse_nlpfr', 'home/coffeehouse_nlpfr')
    >>> split_resource_url('coffeehouse_nlpfr:/home/coffeehouse_nlpfr')
    ('coffeehouse_nlpfr', '/home/coffeehouse_nlpfr')
    >>> split_resource_url('file:/home/coffeehouse_nlpfr')
    ('file', '/home/coffeehouse_nlpfr')
    >>> split_resource_url('file:///home/coffeehouse_nlpfr')
    ('file', '/home/coffeehouse_nlpfr')
    >>> split_resource_url('file:///C:/home/coffeehouse_nlpfr')
    ('file', '/C:/home/coffeehouse_nlpfr')
    """
    protocol, path_ = resource_url.split(":", 1)
    if protocol == "coffeehouse_nlpfr":
        pass
    elif protocol == "file":
        if path_.startswith("/"):
            path_ = "/" + path_.lstrip("/")
...
def normalize_resource_url(resource_url):
    ...
    >>> not windows or normalize_resource_url('file:////C:/dir/file') == 'file:///C:/dir/file'
    True
    >>> not windows or normalize_resource_url('coffeehouse_nlpfr:C:/dir/file') == 'file:///C:/dir/file'
    True
    >>> not windows or normalize_resource_url('coffeehouse_nlpfr:C:\\dir\\file') == 'file:///C:/dir/file'
    True
    >>> windows or normalize_resource_url('file:/dir/file/toy.cfg') == 'file:///dir/file/toy.cfg'
    True
    >>> normalize_resource_url('coffeehouse_nlpfr:home/coffeehouse_nlpfr')
    'coffeehouse_nlpfr:home/coffeehouse_nlpfr'
    >>> windows or normalize_resource_url('coffeehouse_nlpfr:/home/coffeehouse_nlpfr') == 'file:///home/coffeehouse_nlpfr'
    True
    >>> normalize_resource_url('http://example.com/dir/file')
    'http://example.com/dir/file'
    >>> normalize_resource_url('dir/file')
    'coffeehouse_nlpfr:dir/file'
    """
    try:
        protocol, name = split_resource_url(resource_url)
    except ValueError:
        # the resource url has no protocol, use the coffeehouse_nlpfr protocol by default
        protocol = "coffeehouse_nlpfr"
        name = resource_url
    # use file protocol if the path is an absolute path
    if protocol == "coffeehouse_nlpfr" and os.path.isabs(name):
        protocol = "file://"
        ...
        # name is absolute
        name = normalize_resource_name(name, False, None)
    elif protocol == "coffeehouse_nlpfr":
        protocol = "coffeehouse_nlpfr:"
...
def find(resource_name, paths=None):
    ...
    Zip File Handling:
      ...
      - If any element of ``coffeehouse_nlpfr.data.path`` has a ``.zip`` extension,
    ...
    msg = str(
        ...
        ">>> import coffeehouse_nlpfr\n"
        ">>> coffeehouse_nlpfr.download('{resource}')\n"
        "\033[0m"
...
def retrieve(resource_url, filename=None, verbose=True):
    ...
    :param resource_url: A URL specifying where the resource should be
        loaded from.  The default protocol is "coffeehouse_nlpfr:", which searches
...
FORMATS = {
    ...
    "coffeehouse_nlpfr.sem.logic.Expression.fromstring.",
    "logic": "A list of first order logic expressions, parsed with "
    "coffeehouse_nlpfr.sem.logic.LogicParser.  Requires an additional logic_parser "
    "parameter",
    "val": "A semantic valuation, parsed by coffeehouse_nlpfr.sem.Valuation.fromstring.",
...
def load(
    ...
    if format == "raw":
        ...
        import json
        from coffeehouse_nlpfr.jsontags import json_tags
        ...
        if format == "text":
            ...
            resource_val = coffeehouse_nlpfr.grammar.CFG.fromstring(string_data, encoding=encoding)
        elif format == "pcfg":
            resource_val = coffeehouse_nlpfr.grammar.PCFG.fromstring(string_data, encoding=encoding)
        elif format == "fcfg":
            resource_val = coffeehouse_nlpfr.grammar.FeatureGrammar.fromstring(
                string_data,
            ...
            resource_val = coffeehouse_nlpfr.sem.read_logic(
                string_data,
                logic_parser=coffeehouse_nlpfr.sem.logic.LogicParser(),
                encoding=encoding,
            )
        elif format == "logic":
            resource_val = coffeehouse_nlpfr.sem.read_logic(
            ...
            resource_val = coffeehouse_nlpfr.sem.read_valuation(string_data, encoding=encoding)
        else:
            raise AssertionError(
                "Internal NLTK error: Format %s isn't "
                "handled by coffeehouse_nlpfr.data.load()" % (format,)
            )
...
def _open(resource_url):
    ...
    given its resource URL.  If the given resource URL uses the "coffeehouse_nlpfr:"
    protocol, or uses no protocol, then use ``coffeehouse_nlpfr.data.find`` to find
    ...
    if protocol is None or protocol.lower() == "coffeehouse_nlpfr":
