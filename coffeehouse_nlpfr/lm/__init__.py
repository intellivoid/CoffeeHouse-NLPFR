...
from NLTK for this.
    ...
    >>> from coffeehouse_nlpfr.util import bigrams
    >>> list(bigrams(text[0]))
    [('a', 'b'), ('b', 'c')]
...
first sentence.
    ...
    >>> from coffeehouse_nlpfr.util import pad_sequence
...
already set while the other arguments remain the same as for `pad_sequence`.
    ...
    >>> from coffeehouse_nlpfr.lm.preprocessing import pad_both_ends
...
While not the most efficient, it is conceptually simple.
    ...
    >>> from coffeehouse_nlpfr.util import everygrams
...
ngrams) and then combine the sentences into one flat stream of words.
    ...
    >>> from coffeehouse_nlpfr.lm.preprocessing import flatten
...
from coffeehouse_nlpfr.lm.models import (
    MLE,
    Lidstone,
    Laplace,
...
from coffeehouse_nlpfr.lm.counter import NgramCounter
from coffeehouse_nlpfr.lm.vocabulary import Vocabulary
