...
``coffeehouse_nlpfr.sem.evaluate``. The code assumes that the Prolog
...
class Concept(object):
    ...
    def close(self):
        ...
        """
        from coffeehouse_nlpfr.sem import is_rel
...
def sql_query(dbname, query):
    ...
    try:
        path = coffeehouse_nlpfr.data.find(dbname)
...
def _str2records(filename, rel):
    ...
    """
    recs = []
    contents = coffeehouse_nlpfr.data.load("corpora/chat80/%s" % filename, format="text")
...
def make_valuation(concepts, read=False, lexicon=False):
    ...
    if lexicon:
        read = True
    if read:
        from coffeehouse_nlpfr.sem import Valuation
