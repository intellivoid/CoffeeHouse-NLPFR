...
from coffeehouse_nlpfr.internals import Counter
from coffeehouse_nlpfr.tag import UnigramTagger, BigramTagger, TrigramTagger, RegexpTagger
...
from coffeehouse_nlpfr.sem import drt
from coffeehouse_nlpfr.sem import linearlogic
...
class GlueDict(dict):
    ...
    def read_file(self, empty_first=True):
        ...
        try:
            contents = coffeehouse_nlpfr.data.load(
                self.filename, format="text", encoding=self.file_encoding
            )
            # TODO: the above can't handle zip files, but this should anyway be fixed in coffeehouse_nlpfr.data.load()
...
class Glue(object):
    def __init__(
        ...
        from coffeehouse_nlpfr import Prover9
    ...
    def dep_parse(self, sentence):
        ...
        if self.depparser is None:
            from coffeehouse_nlpfr.parse import MaltParser
