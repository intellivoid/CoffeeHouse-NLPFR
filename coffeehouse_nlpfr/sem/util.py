...
import codecs
from coffeehouse_nlpfr.sem import evaluate
...
def parse_sents(inputs, grammar, trace=0):
    ...
    :rtype: list(coffeehouse_nlpfr.tree.Tree) or dict(list(str)): list(Tree)
    :return: a mapping from input sentences to a list of ``Tree``s
    """
    # put imports here to avoid circult dependencies
    from coffeehouse_nlpfr.grammar import FeatureGrammar
    from coffeehouse_nlpfr.parse import FeatureChartParser, load_parser
...
def root_semrep(syntree, semkey="SEM"):
    ...
    """
    from coffeehouse_nlpfr.grammar import FeatStructNonterminal
...
def interpret_sents(inputs, grammar, semkey="SEM", trace=0):
    ...
    :rtype: list(list(tuple(coffeehouse_nlpfr.tree.Tree, coffeehouse_nlpfr.sem.logic.ConstantExpression)))
    """
    return [
...
def evaluate_sents(inputs, grammar, model, assignment, trace=0):
    ...
    :rtype: list(list(tuple(coffeehouse_nlpfr.tree.Tree, coffeehouse_nlpfr.sem.logic.ConstantExpression, bool or dict(str): bool)))
    """
    return [
        [
